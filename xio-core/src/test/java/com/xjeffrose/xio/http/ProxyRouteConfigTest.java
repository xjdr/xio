...
public class ProxyRouteConfigTest extends Assert {
  ...
  public void testBuilderCustomValues() {
    ProxyRouteConfig fallbackObject = new ProxyRouteConfig(ConfigFactory.load().getConfig("xio.proxyRouteTemplate"));
    ...
    List<ClientConfig> expectedClientConfigs = Lists.newArrayList(
      ClientConfig
        .newBuilder(new ClientConfig(ConfigFactory.load().getConfig("xio.clientTemplate")))
        .setName("client config name")
        .build()
    );
    ProxyRouteConfig.ProxyHostPolicy expectedProxyHostPolicy = ProxyRouteConfig.ProxyHostPolicy.UseConfigValue;
    String expectedProxyHost = "proxyHost";
    String expectedProxyPath = "proxyPath";

    ProxyRouteConfig subject = ProxyRouteConfig
