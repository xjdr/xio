...
public class Http2StreamMapper extends ChannelDuplexHandler {
  ...
  private static final AttributeKey<BiMap<Integer, Integer>> STREAM_ID_BI_MAP_KEY =
      AttributeKey.newInstance("xio_stream_id_bi_map_key");
  ...
  public static int getOutboundResponseId(ChannelHandlerContext ctx, int inResponse) {
    ...
    if (responseId == null) {
      throw new RuntimeException(
          "Coudln't find the outbound response stream id for inbound response stream id: "
              + inResponse);
    }
