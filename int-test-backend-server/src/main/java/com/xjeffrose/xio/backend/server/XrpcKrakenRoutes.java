  ...
  XrpcKrakenRoutes(String name) {
    Handler handler =
        (request) -> {
          ...
          Optional<String> echoValue =
              Optional.ofNullable(request.header("x-echo")).map(CharSequence::toString);
          ...
          return request.createResponse(
              HttpResponseStatus.OK, encoded, encoder.mediaType(), customHeaders);
        };
    get("/", handler);
    post("/", handler);
  }
