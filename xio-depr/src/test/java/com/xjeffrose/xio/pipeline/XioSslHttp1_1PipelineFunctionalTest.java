...
public class XioSslHttp1_1PipelineFunctionalTest extends Assert {
  ...
  public void testServer() throws IOException {
    XioServerBootstrap bootstrap =
        XioServerBootstrap.fromConfig("xio.testHttpsServer")
            .addToPipeline(
                new SmartHttpPipeline() {
                  @Override
                  public ChannelHandler getApplicationRouter() {
                    return new PipelineRouter(
                        ImmutableMap.of(), new SamplePipelineRequestHandler());
                  }
                });
              ...
              .addToPipeline(
                  new SmartHttpPipeline() {
                    ...
                    public ChannelHandler getApplicationRouter() {
                      return new PipelineRouter(
                          ImmutableMap.of(),
                          new ProxyPipelineRequestHandler(testServer.boundAddress(), false));
                    }
                  });
